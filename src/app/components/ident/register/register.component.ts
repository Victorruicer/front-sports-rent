...
import { RegistroFailure, RegistroSuccess } from '../redux/store/registro.actions';
  ...
  crear(){
    ...
    this.auth.registro(datosR).subscribe(data => {
      if(data.Retcode === 0){
        ...
        this.toastr.success(data.Nombre + " te has registrado correctamente", "Ya puedes inciar sesión");
      }else{
